      ...
      if vim.o.guifont and vim.o.guifont ~= "" then
        ...
        if vim.g.GuiFont and vim.g.GuiFont ~= "" then
          ...
          if vim.fn.exists("+guifont") == 1 then
            ...
            local cmd_font = vim.api.nvim_exec2("set guifont?", {output = true})
            if cmd_font and cmd_font.output then
...
function M.get_treesitter_info()
  ...
  local ts_parsers = _G.require_and_setup("nvim-treesitter.parsers", false)

  if not ts_parsers then
    table.insert(ts_status, "  Installed: No (or not loaded)")
    table.insert(ts_status, "  Status: Not available - Module not found")
    return ts_status
  end
...
function M.get_command_info()
  return {
    "",
    "Available Commands:",
    "  :TerminalMode - Switch to basic color mode",
...
function M.add_color_test_blocks(buf)
  ...
  vim.api.nvim_buf_set_lines(buf, -1, -1, false, {
    ...
    "• Compares TerminalMode and GUIMode highlighting",
...
function M.add_recommendations(buf)
  ...
  if utils.is_gui_environment() then
    ...
    vim.api.nvim_buf_set_lines(buf, -1, -1, false, {
      ...
      "------------------------------",
      "1. Use :TerminalMode to apply enhanced 256-color mode that works in any modern terminal",
